<?php
/**
 * Moments Layout Template
 * 3-row layout: Title + Icon, Description, Checkbox + Add to Cart text + Price
 *
 * Variables available:
...
<div class="aura-products-wrapper" data-widget-id="<?php echo esc_attr( $widget_id ); ?>">
	<div class="aura-products-grid"
		...
		<?php foreach ( $products as $index => $product ) :
			...
			$is_checked = $initial_quantity > 0;
		?>

		<div class="aura-product-card aura-moments-card <?php echo esc_attr( $is_hidden ); ?>"
			 data-product-id="<?php echo esc_attr( $product_id ); ?>"
			 data-product-price="<?php echo esc_attr( $product_price ); ?>">

			<!-- Row 1: Title (left) + Icon (right) -->
			<div class="aura-moments-header">
				<h3 class="aura-product-title">
					<?php echo esc_html( $product_name ); ?>
				</h3>
				<div class="aura-moments-icon"
			...
			<?php if ( ! empty( $product_description ) ) : ?>
				<div class="aura-moments-description">
					<?php echo wp_kses_post( $product_description ); ?>
				</div>
			<?php endif; ?>

			<!-- Row 3: Checkbox + "Add to Cart" text (left) | Price (right) -->
			<div class="aura-moments-footer">
				<div class="aura-moments-add-wrapper">
					<button class="aura-moments-checkbox-btn"
							data-product-id="<?php echo esc_attr( $product_id ); ?>"
							role="checkbox"
							aria-checked="<?php echo $is_checked ? 'true' : 'false'; ?>"
							aria-label="<?php esc_attr_e( 'Add to cart', 'aura-custom-product-style' ); ?>">
						<span class="aura-moments-checkmark">✓</span>
					</button>
					<span class="aura-moments-add-text">
						<?php esc_html_e( 'Add to Cart', 'aura-custom-product-style' ); ?>
					</span>
				</div>
				<div class="aura-moments-price">
					<strong><?php echo esc_html( get_woocommerce_currency_symbol() ); ?> <?php echo esc_html( number_format( floatval( $product_price ), 2 ) ); ?></strong>
				</div>
			</div>

		</div><!-- .aura-moments-card -->
